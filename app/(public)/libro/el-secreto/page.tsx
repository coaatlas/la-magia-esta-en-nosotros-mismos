...
export default function BookDetailPage() {
  ...
  const book: Book = {
    ...
    title: "El Secreto",
    author: "Rhonda Byrne",
    cover: "/covers/elsecreto.webp",
    description: "El Secreto ha revelado las grandes verdades de la vida a innumerables personas en todo el mundo. Ahora, en esta adaptación para audiolibro, descubrirás cómo aplicar la Ley de la Atracción en cada aspecto de tu vida: salud, dinero, relaciones y felicidad. Todo lo que deseas está a tu alcance, solo necesitas conocer El Secreto.",
    price: { oneTime: 1000 },
    totalDuration: "3h 45min",
    chapters: [
      { id: 'intro', title: 'Introducción', duration: '4:12', durationSeconds: 252, src: '/audios/elsecreto/c.mp3', isPreview: true, description: 'Bienvenido al viaje del descubrimiento' },
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white relative pb-32">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}>
          ...
          {!hasAccess && book.chapters.some(c => !c.isPreview) && (
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.8 }} className="mt-8 p-6 bg-gradient-to-br from-amber-500/10 via-purple-500/5 to-transparent rounded-2xl border border-amber-500/20 text-center">
              <h3 className="text-xl font-bold text-white mb-2">¿Listo para descubrir El Secreto?</h3>
              <p className="text-gray-400 mb-5 max-w-md mx-auto">Desbloqueá los {book.chapters.filter(c => !c.isPreview).length} capítulos premium y comenzá a manifestar la vida que merecés.</p>
      ...
      <footer className="px-6 md:px-8 py-12 text-center text-gray-600 text-sm">
        <p>© {new Date().getFullYear()} El Secreto • Audiolibros</p>
        <p className="mt-1 text-xs opacity-70">Tu realidad es tu creación ✨</p>
      </footer>
